...
1. **`act` field** (declarative) — set `act: act.askUser(...)` on the step config. The SDK prepends the generated prose before the step's prompt.
2. **`act.*` in a prompt function** (composable) — call `act.askUser(...)` etc. inside the prompt callback to control exactly where the primitive prose appears, mix primitives with system segments, or derive primitive config from stash/context.

Both approaches produce identical prose output. Use the `act` field for static configs; use `act.*` in a prompt function when you need composition or dynamic values.
...
- **`act.askUser(...)`, `act.confirm(...)`, `act.plan(...)`, `act.checklist(...)`, `act.subagent(...)`** — an `ActSegment` that the engine resolves to host-aware prose at runtime.

The engine assembles pieces in author order: `resolvePromptValue` evaluates the prompt config, `normalizePieces` wraps the result in an array, then `assemblePieces` renders each piece (system text, primitive prose, or plain string) and joins with double newlines. If an `act` field is also set on the step, it is prepended as the first piece.
