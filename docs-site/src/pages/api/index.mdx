...
All prompt segments are emitted as XML tags in the output: `<system>`, `<prompt>`, `<ask-user>`, `<confirm>`, `<plan>`, `<checklist>`, `<subagent>`, `<rendered>`. No tool names appear in the XML. The preamble (sent once at session start) maps tags to host-specific tools.
...
| `<subagent>`  | Agent           | Spawn isolated agent for enclosed task via the tool. Return its output.                                                        |
...
definePrimitive({
  ...
  render: (config: TConfig) => string,            // produce XML output
...
- **`renderPrimitive(config)`** — renders any `PrimitiveConfig` to its XML tag output.
