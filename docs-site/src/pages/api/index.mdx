...
```typescript
import { step, z, act } from '@contentful/skill-kit';
...
.extend('ask-stack', openQuestion, {
  ...
  act: act.askUser({ type: 'open', question: "What's your tech stack?" }),
  next: 'ask-hobby',
})
```
...
```typescript
{
  prompt?: string | PromptPiece[] | PromptFn,
  act?: ActSegment,                                        // from act.askUser(...), act.confirm(...), etc.
...
`prompt` accepts three forms: a static string, an array of `PromptPiece` values, or a `PromptFn` callback that receives `PromptContext` and returns `PromptReturn` (`string | PromptPiece | PromptPiece[]`). When using a function, the `act` and `system` builders on `PromptContext` let you compose primitives and system directives inline — see [Composable Prompt Vocabulary](#composable-prompt-vocabulary).

`act` is the declarative alternative: set it to an `ActSegment` (returned by `act.askUser(...)`, `act.confirm(...)`, `act.plan(...)`, `act.checklist(...)`, `act.subagent(...)`) and the SDK prepends the rendered XML tag automatically. For dynamic primitive composition (e.g., a `plan()` whose steps depend on stash), use `act.*` inside a prompt function instead. Convention: place `prompt` before `act` in the step config object.
...
| `act`      | `ActBuilder`                                            | Factory for primitive segments — see [Composable Prompt Vocabulary](#composable-prompt-vocabulary)      |
| `system`   | `SystemBuilder`                                         | Factory for system-level directives — see [Composable Prompt Vocabulary](#composable-prompt-vocabulary) |
...
1. **`act` field** (declarative) — set `act: act.askUser(...)` on the step config. The SDK prepends the rendered XML tag before the step's `<prompt>`.
2. **`act.*` in a prompt function** (composable) — call `act.askUser(...)` etc. inside the prompt callback to control exactly where the primitive XML tag appears, mix primitives with system segments, or derive primitive config from stash/context.

Both approaches produce identical XML output. Use the `act` field for static configs; use `act.*` in a prompt function when you need composition or dynamic values.
...
- **`act.askUser(...)`, `act.confirm(...)`, `act.plan(...)`, `act.checklist(...)`, `act.subagent(...)`** — an `ActSegment` that the engine renders to its XML tag at runtime via `renderPrimitive()`.

The engine assembles pieces in author order: `resolvePromptValue` evaluates the prompt config, `normalizePieces` wraps the result in an array, then `assemblePieces` wraps each piece in its XML tag (`<prompt>` for strings, `<system>` for system segments, primitive-specific tags like `<ask-user>` or `<checklist>` for act segments) and joins with double newlines. If an `act` field is also set on the step, it is prepended as the first piece.

### XML output format

All prompt segments are emitted as XML tags in the output: `<system>`, `<prompt>`, `<ask-user>`, `<confirm>`, `<plan>`, `<checklist>`, `<subagent>`, `<rendered>`. No tool names appear in the XML. The `<subagent>` tag may include a `no-recurse` attribute naming the skill that the subagent must not invoke (see [`allowRecursion`](#subagentconfig) above). The preamble (sent once at session start) maps tags to host-specific tools.

The preamble is a markdown table with columns: **Tag**, **Tool**, **How to use**. Each primitive's `preambleRow()` method generates its row, with the tool column populated by `resolveTools()` which uses either/or resolution: if the agent reports any explicit tools (via `--tools`), those are authoritative for all primitives; otherwise the host registry provides the tool list.
...
- **`resolveTools(handshake)`** — either/or resolution at the whole-handshake level: if the agent reports any explicit tools, they are authoritative for all primitives (no registry merge); otherwise the host registry provides the tool list. Returns a `ToolResolver` map from tag name to matched tool (or `undefined`).
...
export default skill({
  ...
  .step('choose', {
    prompt: 'Ask which environment to deploy to.',
    act: act.askUser({
      type: 'structured',
      question: 'Which environment?',
      options: [
