...
```typescript
import { step, z, askUser } from '@contentful/skill-kit';
...
.extend('ask-stack', openQuestion, {
  primitive: askUser({ type: 'open', question: "What's your tech stack?" }),
...
`primitive` is the declarative alternative: set it to a `PrimitiveConfig` (returned by `askUser()`, `confirm()`, `plan()`, `checklist()`, `subagent()`) and the SDK prepends the generated prose automatically. For dynamic primitive composition (e.g., a `plan()` whose steps depend on stash), use `act.*` inside a prompt function instead.
...
1. **`primitive` field** (declarative) — set `primitive: askUser(...)` on the step config. The SDK prepends the generated prose before the step's prompt.
...
Both approaches produce identical prose output. Use `primitive` for static configs; use `act.*` when you need composition or dynamic values.
