...
The preamble is a markdown table with columns: **Tag**, **Tool**, **How to use**. Each primitive's `preambleRow()` method generates its row, with the tool column populated by `resolveTools()` which matches the primitive's tool list against the host's available tools.
...
- **`resolveTools(handshake)`** — matches each primitive's tool list against the host's available tools. Returns a `ToolResolver` map from tag name to matched tool (or `undefined`).
