...
```typescript
{
  ...
  action?: {
    ...
    input?: (ctx: { output; stash }) => unknown,
...
The `stash` callback receives `{ output, action? }` and returns a partial stash object. The return value is shallow-merged into the accumulated stash:
...
.step('save', {
  ...
  action: {
    ...
    stash: ({ action }) => ({ savedPath: action.path }),
  },
...
- `action.stash` stashes action results (runs after action, before transition)
