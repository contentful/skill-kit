...
const result = await runSkill(mySkill, {
  model: mockModel({ /* step responses */ }),
...
Interactive testing adapter for manual evaluation — you provide responses in real time instead of using canned values. Not yet implemented in v0.1; use `mockModel` for now.
