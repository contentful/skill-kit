...
export default skill({
  ...
  .step('classify', {
    ...
    next: ({ stepOutput }) => {
      ...
      if (stepOutput.intent === 'doctor') return 'get-space';
...
Each `.subskill()` registration accepts an optional `params` function that maps the dispatcher's state (step output + stash) to the sub-skill's params.
