...
| `contextBudget` | `'narrow' \| 'normal' \| 'wide'` | No       | Hint for context window allocation. Defaults to `'normal'`. |

On hosts without real agent isolation (no `Agent` tool), the prose fallback still produces correct output but doesn't get the context-window benefit.
