...
Module steps work identically to skill steps, with one difference: they receive the **module's** stash type, not the parent skill's. The `params` parameter is typed as `unknown` since modules are params-independent.
...
module({
  ...
  .step('auth-verify', {
    ...
    updateStash: ({ stepOutput }) => ({ token: stepOutput.token }),
...
export const authModule = module({
  ...
  .step('auth-login', {
    ...
    updateStash: ({ stepOutput }) => ({ authMethod: stepOutput.method }),
  ...
  .step('auth-credentials', {
    ...
    updateStash: ({ stepOutput }) => ({ userId: stepOutput.userId }),
    next: ({ stepOutput }) => (stepOutput.success ? 'auth-confirm' : 'auth-login'),
    maxVisits: 3,
...
export default skill({
  ...
  .step('welcome', {
    ...
    updateStash: ({ stepOutput }) => ({ projectName: stepOutput.projectName }),
    next: 'auth-login',
  })
