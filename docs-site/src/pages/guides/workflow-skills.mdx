...
.step('gather-info', {
  ...
  action: {
    ...
    input: ({ output, stash }) => ({ checks: output.checks }),
    stash: ({ action }) => ({ reportPath: action.path }),
  },
...
| `act`         | `ActSegment`                                      | No       | Shorthand for single-primitive steps (from `act.askUser(...)`, etc.).    |
...
| `stash`       | `(ctx: { output, action? }) => Partial<Stash>`    | No       | Shallow-merge updates into the cross-step scratchpad.                    |
| `action`      | `{ run, input?, stash? }`                         | No       | CLI-side side effect that runs after output validation.                  |
...
The stash exposed in prompt functions is a **frozen** (read-only) snapshot. Mutations are only possible through `stash` and `action.stash` callbacks — never inside prompts.
