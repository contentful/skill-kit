...
**Per-step prose generation.** For any step using a primitive (via the `primitive` field or `act.*` in a prompt function), the SDK generates prose calibrated to the current host. On Claude Code, an `askUser` step emits:
...
3. **Prepend `primitive`** — If the step config has a `primitive` field, an `ActSegment` wrapping it is unshifted to the front of the pieces array. This means declarative primitives always appear before prompt content.
4. **`assemblePieces`** — Walks the array in author order, rendering each piece:
   - Plain strings pass through.
   - `SystemSegment` (`{ kind: 'system' }`) emits its text.
   - `ActSegment` (`{ kind: 'act' }`) is resolved to host-aware prose via the `ProseGenerator`.

   Non-empty results are joined with double newlines.

This pipeline means the author controls ordering. A prompt function returning `[system`...`, act.checklist(...), 'Build the game.']` produces exactly that sequence in the final prompt. The `primitive` field is a convenience that always prepends; use `act.*` when ordering matters.
