...
- **`resolveTools(handshake)`** — for each primitive, finds the first tool from its `tools` list that appears in `handshake.toolsAvailable` (explicit), falling back to the host registry. Returns a `ToolResolver` map from tag name to matched tool (or `undefined`).

Tool resolution uses a two-pass strategy: first check the handshake's explicit `toolsAvailable` list, then fall back to the `HOST_REGISTRY` for the given host name. This means a host can advertise tools the SDK doesn't know about, and the SDK will still match them if a primitive lists that tool name.
