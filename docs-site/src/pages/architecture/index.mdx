...
| `--tools`       | Optional     | Comma-separated list of available tools (overrides host registry). Only needed on `start` — session mode stores tools in the session header and reads them back on `advance` |
...
**Per-step XML rendering.** For any step using a primitive (via the `act` field or `act.*` in a prompt function), the SDK renders the primitive as an XML tag. An `askUser` step emits:
...
- **`resolveTools(handshake)`** — either/or resolution at the whole-handshake level: if the agent reports any explicit tools (via `--tools`), those are authoritative for all primitives (no registry merge); if no tools are reported, the host registry provides the tool list. Returns a `ToolResolver` map from tag name to matched tool (or `undefined`).

Tool resolution is either/or at the whole-handshake level: if the agent reports any explicit tools (via `--tools`), those are the authoritative tool list for all primitives — no registry fallback. If no explicit tools are reported, the `HOST_REGISTRY` for the given host name provides the tool list. This prevents subagents (which report a subset of tools but still identify as a known host) from being told to use tools they don't actually have.
...
1. **`resolvePromptValue`** — Evaluates the step's `prompt` config. If it's a function, calls it with `PromptContext` (which includes `act` and `system` builders). If it's a string or array, uses it directly. Returns `PromptReturn`.
...
3. **Prepend `act`** — If the step config has an `act` field, the `ActSegment` is unshifted to the front of the pieces array. This means declarative primitives always appear before prompt content.
4. **`assemblePieces`** — Walks the array in author order, wrapping each piece in its XML tag:
...
This pipeline means the author controls ordering. A prompt function returning `[system`...`, act.checklist(...), 'Build the game.']` produces `<system>...</system>`, then `<checklist>...</checklist>`, then `<prompt>Build the game.</prompt>` in the final output. The `act` field is a convenience that always prepends; use `act.*` in a prompt function when ordering matters.
...
**Constructor** — Takes a `SkillDefinition`, `Handshake`, context, and optional `ReferenceLoader`. Initializes the stash store and tool resolver.
...
5. **action.stash** — if the step has `action.stash`, merge action results into the stash.
6. **Merge stash** via the step's `stash()` callback (if present). Receives `{ output, action? }`. Shallow merge into accumulator.
