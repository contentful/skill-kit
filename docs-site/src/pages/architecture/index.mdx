...
**Per-step prose generation.** For any step using a primitive (via the `act` field or `act.*` in a prompt function), the SDK generates prose calibrated to the current host. On Claude Code, an `askUser` step emits:
...
This pipeline means the author controls ordering. A prompt function returning `[system`...`, act.checklist(...), 'Build the game.']` produces exactly that sequence in the final prompt. The `act` field is a convenience that always prepends; use `act.*` in a prompt function when ordering matters.
