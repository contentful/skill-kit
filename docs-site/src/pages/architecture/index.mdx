...
History replay is cheap. The engine reconstructs state (store, visit counts) from history data without re-executing actions or observers.
...
**Constructor** — Takes a `SkillDefinition`, `Handshake`, params, and optional `ReferenceLoader`. Initializes the state store and tool resolver.
...
1. **Validate** output against step's ArkType schema via `safeParse()`.
...
Append-only store. Each step appends a `StepResult` record containing the step name, response, actionResult, and computed result. The store provides a typed accessor (`StoreView`) via a `Proxy` that maps property access to step-name lookups. The SDK analyzes the workflow graph at build time to determine which steps are guaranteed (on all paths from entry) vs optional (branch targets). Guaranteed steps are non-optional in the type system; branch targets require `?.`. Methods like `all(step)` and `ran(step)` provide loop and existence queries. Values are frozen after append.
