...
The params function receives the dispatching step's output and the dispatcher's accumulated stash. Its return value becomes the sub-skill's params.
...
The step's `action.updateStash` callback stashes the results, and `next` routes based on the action output — either to guided setup or directly to configuration.
