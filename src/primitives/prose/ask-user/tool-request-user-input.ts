...
export function toolRequestUserInputProse(config: AskUserConfig): string {
  if (config.type === 'open') {
  ...
  return [
    `ASK_STRUCTURED: Use the ToolRequestUserInput tool to ask the user: "${config.question}"`,
    `Provide these as the options array: ${optionsList}.`,
