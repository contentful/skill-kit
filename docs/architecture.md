...
**Per-step XML rendering.** For any step using a primitive (via `act` methods in the prompt function or by passing an `ActSegment` directly to `prompt`), the SDK renders the primitive as an XML tag. On any host, an `askUser` step emits:
...
1. **`resolvePromptValue`** — calls the prompt function (or returns a static string/`PromptPiece`/`PromptPiece[]`) to get the raw `PromptReturn`.
2. **`normalizePieces`** — coerces the return value to an array of `PromptPiece` objects (strings, system segments, act segments, view segments).
3. **`assemblePieces`** — iterates pieces in author order and wraps each as XML:
   ...
   - Act segments are rendered via `renderPrimitive()` into their respective XML tags (e.g., `<ask-user>`, `<plan>`, `<checklist>`, `<survey>`)
   - View segments (from the `view()` helper) become `<rendered>\n...\n</rendered>` with an optional `name` attribute (e.g., `<rendered name="report">`)
   - All pieces are concatenated in the order written

Since `prompt` accepts `PromptPiece` directly, single-primitive steps pass an `ActSegment` straight to `prompt` without needing a wrapper function.
