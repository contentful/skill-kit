...
The preamble is generated by `preambleRows()` in the registry, which calls each primitive's `preambleRow(tool)` method with the resolved tool name (or `undefined`). Tool resolution is per-primitive: explicit `--tools` first, then host registry fallback via `resolveTools(handshake)`.
